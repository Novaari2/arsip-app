...
@include('layouts.overview', ['text' => 'Pejabat Lelang', 'icon' => 'mdi mdi-airplay'])
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <a href="{{ route('pejabat_lelang.create') }}" class="btn btn-sm bg-green text-white aadd mb-4"><i class="mdi mdi-airplus"></i>Tambah</a>
...
<script>
    ...
    let table = $('.data-all').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('pejabat_lelang.index') }}",
        columns:[
            {
                data: "nama",
                name: "nama"
    ...
    $(document).on('click','.delete-pejabat', function() {
            let id = $(this).data('id');
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('pejabat_lelang.destroy') }}",
                        type: "DELETE",
